import builtins, func_timeout, inspect, logging, multiprocessing, os, pickle, sys, traceback,json,time
...
class ExplorationEngine:
    ...
    def __init__(self, *, solver='cvc4', timeout=10, safety=0, store=None, verbose=1, logfile=None, statsdir=False,smtdir=""
                 ...
                 module=None, execute_):
        global execute

        # module = module_ 0510暫時註解
        execute = execute_
    ...
    def _execution_loop(self, max_iterations, all_args,start_time,timeout):
        ...
        while cont and (max_iterations==0 or iterations<max_iterations) and len(self.constraints_to_solve) > 0 and time.time()<start_time+timeout:
            
            log.info(f'[{self.funcname}][PyCT_HAS_CONSTRAINT]: After First execution, start trying solve constraints')
            ##############################################################
            ...
            if model is not None: #sat, model is none->unsat
                ...
                if all_args not in tried_input_args:
                    ...
                    iterations += 1
                    log.info(f"=== [{self.funcname}] Iterations: {iterations}, find new arg ===")
                    #---------------------印出新解出的args-----------------------------
                    log.debug(f"new arg:{all_args}")
        ...
        if time.time()>=start_time+timeout:
            logging.info(f"execution_loop time_out")
        return iterations
    ...
    def explore(self, modpath, all_args={}, /, *, root='.', funcname=None, max_iterations=0, single_timeout=15, total_timeout=60
        ...
        try:
            ...
            iterations = self._execution_loop(max_iterations, all_args,start_time=current_time,timeout=self.total_timeout)
            ...
            if self.statsdir:
                input_list=[(e[0],e[1]!=self.original_answer) for e in self.in_out]
                with open(self.statsdir + '/inputs.pkl', 'wb') as f:
                    pickle.dump(input_list, f) # store only inputs
    ...
    def _one_execution(self, all_args):
        ...
        if self.original_answer == None:
            self.original_answer=self._one_execution_primitive(self.original_args)
    ...
    def _one_execution_primitive(self, all_args):
        ...
        def child_process():
            ...
            # 0529暫時註解
            import coverage
            cov = coverage.Coverage(data_file='/home/soslab/pyct-coverage/coverage_ray/transformers/.coverage',auto_data=True) # Create a coverage instance
            cov.load()
