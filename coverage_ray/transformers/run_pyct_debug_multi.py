...
def run_pyct_one_func(rootdir, file, funcname, lib, TOTAL_TIMEOUT, verbose, in_dict):
    ...
    pyct.run(root=rootdir, modpath=relative_modpath, funcname=funcname,
            ...
            single_timeout=3600, total_timeout=TOTAL_TIMEOUT, timeout=TOTAL_TIMEOUT, verbose=verbose,)
    cov.stop() # Stop measuring coverage
...
def run_explore_one_func_no_coverage(rootdir, file, funcname, lib, TOTAL_TIMEOUT, verbose, in_dict,logfile=None,func=None):
    rootdir = Path(rootdir)
    file = Path(file)
    relative_modpath = str(file.relative_to(rootdir).with_suffix(''))
    ...
    import sys
    sys.path.append('/home/soslab/pyct-coverage/coverage_ray/PyCT')
    ...
    time=f'{time_result.tm_mon}_{time_result.tm_mday}'
    # statsdir = os.path.join("/home/soslab/pyct-coverage/coverage/transformers/ct_log", time, relative_modpath,funcname)
    statsdir = os.path.join("/home/soslab/pyct-coverage/coverage_ray/transformers/ct_log", time, relative_modpath,funcname)

    # 0510暫時註解(換目錄好像會讓tracer出問題)
    ...
    engine = explore.ExplorationEngine(solver='cvc4', timeout=TOTAL_TIMEOUT, safety=safety,
                                            ...
                                            statsdir=statsdir, execute_=execute,only_first_forward=False)


    result = engine.explore(
        relative_modpath, in_dict, root=rootdir, funcname=funcname, max_iterations=0,
        single_timeout=TOTAL_TIMEOUT/5, total_timeout=TOTAL_TIMEOUT, deadcode=set(),
        include_exception=True, lib=lib,
        file_as_total=file_as_total)
    
    total_iter = result[0]
    
    logging.info(f"Total iterations:{total_iter}")
